...
group by descr
union all
select '3', 'Actual vs Budget %' "descr",
...
from 
(
select month, sum(act) act, sum(bgt) bgt, sum(sen)sen, sum(pot)pot from view_produksi_tbs_consol 
where year = :p_year
AND SITE = DECODE(:p_site,null,SITE,:p_site)
and plantingyear = decode(:p_plant,null, plantingyear, :p_plant)
and estatecode = decode(:p_estate,null, estatecode, :p_estate)
...
union all
select '5','Actual vs Sensus %' "descr",
...
union all
select '7', 'Actual vs Potensi %' "descr",
...
const fetchData = async function (users, routes, params, callback) {
  ...
  binds.p_estate = (!params.p_estate ? '' : params.p_estate)
  binds.p_division = (!params.p_division ? '' : params.p_division)
