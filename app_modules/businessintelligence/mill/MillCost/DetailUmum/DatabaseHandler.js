...
const baseQuery = `SELECT   jobcode||' - '||getjob_des(jobcode)  "gruptipe",
...
FROM   (  SELECT   'ACT' kat,
     ...
     FROM   tbactivity_CONSOL
    WHERE       substr(jobcode,1,1) in ('6','7') and targetcode in ('31BU','31PR','31QC','3199') 
            AND fiscalyear = :p_year
            AND site = DECODE (:p_site, 'USTP', site, :p_site)
 GROUP BY   jobcode, fiscalyear, period
 UNION ALL
   SELECT   'BGT' kat,
            fiscalyear,
            period,
            jobcode, 
            SUM (amount) amount
     FROM   budgettbactivity_CONSOL
    WHERE       substr(jobcode,1,1) in ('6','7') and targetcode in ('31BU','31PR','31QC','3199') 
            AND fiscalyear = :p_year
            AND site = DECODE (:p_site, 'USTP', site, :p_site)
 GROUP BY   jobcode, fiscalyear, period)
GROUP BY   jobcode, kat
UNION ALL
SELECT   jobcode||' - '||getjob_des(jobcode)  gruptipe,
'3. Achievement (%)' grup,
