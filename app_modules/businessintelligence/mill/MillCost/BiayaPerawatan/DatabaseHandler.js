...
const baseQuery = `SELECT gruptipe "gruptipe",
tipe "tipe",
b_1"b_1",
b_2"b_2",
b_3"b_3",
...
SELECT DECODE (
        detkat,
        'OLAH', 'Biaya Pengolahan',
        'MAIN', 'Biaya Perawatan',
        '3. Biaya Umum PKS'
    ) gruptipe,
...
FROM (
 ...
 WHERE year = :p_year AND site = :p_site AND detkat = 'MAIN'
)
GROUP BY detkat, kat
UNION ALL
SELECT DECODE (
    ...
    '3. Achievement (%)' grup,
    SUM (DECODE (kat || period, 'ACT1', amount)) / SUM (DECODE (kat || period, 'BGT1', amount)) * 100 b_1,
    SUM (DECODE (kat || period, 'ACT2', amount)) / SUM (DECODE (kat || period, 'BGT2', amount)) * 100 b_2,
    SUM (DECODE (kat || period, 'ACT3', amount)) / SUM (DECODE (kat || period, 'BGT3', amount)) * 100 b_3,
    SUM (DECODE (kat || period, 'ACT4', amount)) / SUM (DECODE (kat || period, 'BGT4', amount)) * 100 b_4,
    SUM (DECODE (kat || period, 'ACT5', amount)) / SUM (DECODE (kat || period, 'BGT5', amount)) * 100 b_5,
    SUM (DECODE (kat || period, 'ACT6', amount)) / SUM (DECODE (kat || period, 'BGT6', amount)) * 100 b_6,
    SUM (DECODE (kat || period, 'ACT7', amount)) / SUM (DECODE (kat || period, 'BGT7', amount)) * 100 b_7,
    SUM (DECODE (kat || period, 'ACT8', amount)) / SUM (DECODE (kat || period, 'BGT8', amount)) * 100 b_8,
    SUM (DECODE (kat || period, 'ACT9', amount)) / SUM (DECODE (kat || period, 'BGT9', amount)) * 100 b_9,
    SUM (DECODE (kat || period, 'ACT10', amount)) / SUM (DECODE (kat || period, 'BGT10', amount)) * 100 b_10,
    SUM (DECODE (kat || period, 'ACT11', amount)) / SUM (DECODE (kat || period, 'BGT11', amount)) * 100 b_11,
    SUM (DECODE (kat || period, 'ACT12', amount)) / SUM (DECODE (kat || period, 'BGT12', amount)) * 100 b_12,
    SUM (DECODE (kat, 'ACT', amount)) / SUM (DECODE (kat, 'BGT', amount)) * 100 b_tot
FROM (
 SELECT SITE,
        KAT,
        DETKAT,
...
// const detailQuery=`SELECT   detkat "gruptipe",
// DECODE (kat, 'ACT', '1. Actual', '2. Budget') "tipe",
// SUM (DECODE (period, 1, amount)) "b_1",
// SUM (DECODE (period, 2, amount)) "b_2",
// SUM (DECODE (period, 3, amount)) "b_3",
...
const fetchDataHeader = async function (users, routes, params, callback) {
  binds = {};
...
//      */
//   binds.p_year = !params.p_year ? '': params.p_year;
//   binds.p_site = !params.p_site ? '': params.p_;
