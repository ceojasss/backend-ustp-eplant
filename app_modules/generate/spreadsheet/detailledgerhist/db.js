...
const fetchDataDynamic = async function (users, find, callback) {
    ...
    // custom code
    if (find.report === 'M') {
        fillStatement = `begin RPT_DETAIL_LEDGER_TMONTH_HIST ( :P_MONTH, :P_YEAR, :P_JOBCODE1, :P_JOBCODE2 ); end;`
    } else {
        fillStatement = `begin RPT_DETAIL_LEDGER_TODATE_HIST ( :P_MONTH, :P_YEAR, :P_JOBCODE1, :P_JOBCODE2 ); end;`
    }
