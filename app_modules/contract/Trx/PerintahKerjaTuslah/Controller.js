...
const { UpdateResult, UpdateDetail } = require('../../../../util/HelperUtility')
const CrudhandlerDB = require('../../../../oradb/dbCrudHandler')


const table = ['CONTRACTAGREEMENT_CTL', 'AGREEMENTDETAIL_CTL', 'HARGA_BBM']
...
async function getDetail(req, res, next) {
    ...
    await HandlerDB.fetchDataDetail(req.user, req.route.path.replace("/", ""), req.query, async (error, result) => {
        ...
        const re_result = await UpdateResult(result)
...
async function post(req, res, next) {
    ...
    const document = req.user.loginid.match(/^.*HO$/) ? 'CONTRACT AGREEMENT HO' : 'CONTRACT AGREEMENT'

    try {
        await CrudhandlerDB.createmstdetailsub(table, document, req.user, req.body, binds)
            .then((result) => {
...
async function put(req, res, next) {
    ...
    try {
        ...
        await CrudhandlerDB.insertupdatemasterdetailsub(table, req.user, req.body, binds)
