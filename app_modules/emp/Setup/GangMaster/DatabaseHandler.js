...
from gang x 
where (departmentcode LIKE  UPPER('%' || :search ||'%') OR divisioncode LIKE  UPPER('%' || :search ||'%')) and mandorecode is not null
order by departmentcode, divisioncode`

const detailQuery = `select rowid "rowid",departmentcode "departmentcode", divisioncode "divisioncode", gangcode "gangcode", gangtype "gangtype", description "description", mandorecode "mandorecode", mandore1code "mandore1code", keranicode "keranicode",  
