...
async function post(req, res, next) {
    ...
    const stmt = req
    const binds = req
    let table = 'vehicleavailability_new'
    let rows
    let errors
    let retval
    let process = []
...
async function init_vh({ user, body }, res, next) {

    let rows

    await HandlerDB.generateVH(user, body).then((result) => {
...
module.exports.init_vh = init_vh;